...
router.get(['/json/search/componentsByTypeAndLocation/:typeFormId/:location/:toothStripStatus/:conformanceStatus/:qaChecksStatus', '/api/search/componentsByTypeAndLocation/:typeFormId/:location/:toothStripStatus/:conformanceStatus/:qaChecksStatus'], async function (req, res, next) {
  try {
    // Retrieve a list of components that match the specified record details
    // Geometry boards and grounding mesh panels are grouped by part number, other component types are ungrouped
    const components = await Search_OtherComponents.componentsByTypeAndLocation(req.params.typeFormId, req.params.location, req.params.toothStripStatus, req.params.conformanceStatus, req.params.qaChecksStatus);
...
router.get(['/json/search/actionsByReferencedUUID/:uuid/:actionType', '/api/search/actionsByReferencedUUID/:uuid/:actionType'], async function (req, res, next) {
  try {
    ...
    } else if (req.params.actionType === 'GeometryBoardRejection') {
