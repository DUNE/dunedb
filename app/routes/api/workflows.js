...
router.get('/workflow/:workflowId', permissions.checkPermissionJson('workflows:view'), async function (req, res, next) {
  try {
...
router.get('/workflows/:typeFormId/list', permissions.checkPermissionJson('workflows:view'), async function (req, res, next) {
  try {
    ...
    return res.status(201).json([workflowIDs, workflowStatuses]);
  } catch (err) {
    logger.info({ route: req.route.path }, err.message);
    res.status(500).json({ error: err.toString() });
  }
