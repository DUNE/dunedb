...
document.getElementById('NewTypeButton').addEventListener('click', function () {
  window.location.href = `/workflowTypes/${document.getElementById('typeFormId').value}/new`;
});
...
document.getElementById('typeFormId').addEventListener('keyup', function (e) {
  if (e.key === 'Enter') window.location.href = `/workflowTypes/${this.value}/new`;
});
