...
function mapRelationships()
{
	...
	var uuid1str = uuid1.toString().substr(6,36); // converts to 'UUID(\"abcdabcd-....\")' and extracts the juice.
  ...
  var uuid_regex = /^[A-Fa-f0-9]{8}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{12}/;
	...
	function link(uuid2,path){
				var component_link_from = {
					componentUuid: uuid1.toString().substr(6,36), // needs to be text, not binary. This is a pain.
					validity,
					type,
					name,
					id,
	...
	function recurse(thing,path) {
		...
		for(var i in thing) {
			if (Array.isArray(thing[i])) {
				...
				link(UUID(thing[i].toString().substr(6,36)),path)
			}
