...
async function saveJobData(record, ip, user)
{
  ...
  if(!record.formId) throw("formId not defined");
  ...
  console.log("saveJobData",record.formId,record.state);
...
async function listRecentJobs(formId,N)
{
...
async function listUserDrafts(user_id)
{
  ...
  var p = await db.collection("jobs")
                    .find(query)
                    .project({formId: 1, "form.form_title": 1, insertDate:1, user:1, _id:1})
                    .toArray();
