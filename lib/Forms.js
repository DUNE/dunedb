...
async function saveForm(form_id,record,collection,ip,user)
{
  ...
  new_record.recordType = 'form';
  new_record.collection = collection;

  new_record.insertDate= new Date();
  new_record.submit_ip= ip;
  const {user_id, displayName, emails} = ser;  // equivalent to user_id = user.user_id, etc
  new_record.user = {user_id, displayName, emails};  // destructuring verison. I like it.
