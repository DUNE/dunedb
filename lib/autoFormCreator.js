...
function autoSchemaCreator(documents, recurse)
{
...
function automaticallyCreateSchema(records) {
  var documents = [];
  for(var record of records) {
    if(record.data) documents.push(record.data);
  }
  return autoSchemaCreator(documents,false);
}
...
if (typeof require !== 'undefined' && require.main === module) 
{
  ...
  logger.info(JSON.stringify(automaticallyCreateSchema(docs),null,2));
}