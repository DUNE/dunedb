...
async function attach_to_database()
{
 ...
 //    }
	...
	try {
		console.log('attaching...',...global.config.mongo_connection);
...
async function initialize_database(db)
{
	...
	console.log("init db");
	 ...
	 if(status_obj.version < 4.2) {
	 	// Minor updates to version 4.2
	 	...
	 	// set type = data.type
	 	db.collection("components").updateMany({type:{$exists: false}},[{$set:{type:"$data.type"}}]);
	 	db.collection("components").updateMany({},{$unset: {"data.type":""}});
	 	...
	 	for(var collection of ['tests',]) {
	   	...
	   	status_obj.version =4.2;
	  }

	  if(status_obj.version < 4.3) {
		 	// Minor updates to version 4.3
		 	// Ensure all tests have got componentUUID set.
		 	await db.collection('tests').deleteMany({componentUuid:{$exists:false}});
		 	db.collection("components").dropIndex("$**_text");
		 	db.collection("components").createIndex({"$**": "text"},{"weights":{"type":3,"data.name":2}});
	   	status_obj.version =4.3;
		}

	  if(status_obj.version < 4.4) {
		 	// Minor updates to version 4.4
		 	// Change name of collection.
		 	await db.renameCollection('componentForm','componentForms');
	   	status_obj.version =4.4;
		}
	  if(status_obj.version < 4.5) {
		 	db.collection("tests").createIndex({"$**": "text"},{"weights":{"formId":3,"componentUuid":2}});
		 	db.collection("jobs").createIndex({"$**": "text"},{"weights":{"formId":3,"componentUuid":2}});
	   	status_obj.version =4.5;
		}


	  if(status_obj.version < 4.6) {
	  	// indices on searchable things in data.
		 	db.collection("components").createIndex({"data.$**": "1"});
		 	db.collection("tests").createIndex({"data.$**": "1"});
		 	db.collection("jobs").createIndex({"data.$**": "1"});
	   	status_obj.version =4.6;
		}
