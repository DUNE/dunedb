...
async function initialize_database(db)
{
	...
	if(status_obj.version < 4.9) 
	{
		...
		await db.collection("components").createIndex({"data.$**": 1});
	 	await db.collection("components").createIndex({"$**": "text"},{"weights":{"type":3,"data.name":2}});

		await db.collection("jobs").dropIndexes(); // often cruft in early versions.
		...
		await db.collection("tests").createIndex({"data.$**": 1});
	 	await db.collection("tests").createIndex({"$**": "text"},{"weights":{"type":3,"data.name":2}});
  ...
  if(status_obj.version < 5.0) {
	...
	console.log("Database schema version",status_obj.version);
};
