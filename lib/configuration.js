...
function load_config_file(inpath)
{
    ...
    if(fs.existsSync(loadpath)) {
      ...
      console.log("{msg: 'Loaded config file "+loadpath+"}");
      return true;
    }
    return false;
}
...
if(!global.config) {
  ...
  // First, defaults.
  load_config_file(Path.join(__dirname,"../config/defaults.js"));
  ...
  var config_files = Glob.sync(config_directory+"/"+"*.js");
  ...
  load_config_file(Path.join(__dirname,"../config/config.js"));
