...
function getComponentTypes(course) {
  var types = [];
  for (var step of course.path) {
    console.log(step)
    if(step.type == "component") {
      console.log("step is a component");
      types.push(step.formId);
    }
  }
  return types
}
...
async function getCourseForComponentType(type)
{
  ...
  for(courseId in list) {
    var path = list[courseId].path;
    for (elem in path) {
      if(elem.type == "component" && elem.formId==type) {
        return courseId
      }
    }
  }
