...
async function retrieve(docId,options) {
  if(!docId) throw new Error("Docs::retrieve() no docId given.");
  var query = {docId: docId};

  if(options && options.version) query["validity.version"] = options.version;
...
Cache.add("docList",async function(){
    ...
    aggregation_stages.push(   { $group: {_id: { docId : "$docId" },
                                          ...
                                          created: { "$last": "$insertion.insertDate" },
                                          version: { "$first": "$validity.version" },
                                          edited_by: {"$first": "$insertion.user"} ,                              
                                          created_by: {"$last": "$insertion.user"} ,                              
                                        } } );
