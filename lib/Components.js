...
async function retrieve(componentUuid,onDate,rollbackDate,projection) {
  // Find the right component
  // rollbackdate and onDate must be in native Date() format or null
  if(!componentUuid) throw new Error("Components::retrieve() no componentUuid given.");
  ...
  // console.log(chalk.blue('---retrieve',...arguments, query));
  // console.log("retrieve",...arguments,query);
...
async function findUuidStartsWith(uuid_string)
{
  ...
  var matches = await db.collection("components")
    ...
    .toArray();
  if(matches.length>20) return []; // too many!
  // console.log(matches);
...
{
  // console.log("Components::search()",txt,match)
  var matchobj= match || {};
  var result = [];
  if(txt) {
    matchobj["$text"] = {$search: txt};
    console.log("Components::search  text search",matchobj);
    result = await 
      ...
      console.log("Components::search non-text search",matchobj);
      result = await 
        db.collection('components').aggregate([      
          // { $match: {$text: {$search: txt}}},
          { $match: matchobj },
