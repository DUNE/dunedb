...
async function retrieveComponent(componentUuid,onDate,rollbackDate,projection) {
  // Find the right component
  // rollbackdate and onDate must be in native Date() format or null
  if(!componentUuid) throw new Error("Components::retrieveComponent() no componentUuid given.");
  ...
  // console.log(chalk.blue('---retrieveComponent',...arguments, query));
...
// Get all types, with counts:
async function listAllOfType(type,limit)
{
    var match_condition = {"type": type};
    var aggregation_stages = [];
    if(type) aggregation_stages.push( { $match: match_condition }  );
