...
TestsModule.prototype.findTestIdStartsWith = async function(id_string,max)
{
  // binary version.
  max = max || 10;
  ...
  console.log(bitlow,bithigh);
  // logger.info(qlow,qhigh,bitlow,bithigh);
 
  var matches = await db.collection("tests")
    .find({_id:{$gte: bitlow, $lte: bithigh}})  // binary ObjectIds only
