...
if (require.main === module) {
  ...
  logger.info("Generating new M2M user for");
  logger.info("        email:",argv.email);
  logger.info("         name:",argv.name);
  logger.info("  permissions:",argv.permissions || "auto");
  // test().then((result)=>{logger.info(result)});
  (async function(){
    ...
    logger.info('added a new machine user:',rec);
    logger.info("\n\n\n");
    ...
    logger.info(JSON.stringify(output_record,null,2));
    database.shutdown();
  })();
}
