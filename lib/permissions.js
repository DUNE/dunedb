...
function userHas(user,permission_required)
{
  if(global.config.deployment='devsite'||'laptop') permission_required = "dev:"+permission_required;
  return ((user||{}).permissions||[]).includes(permission_required);
}
...
function hasPermission(req,permission_required)
{
  ...
  return userHas(req.user,permission_required);
}
