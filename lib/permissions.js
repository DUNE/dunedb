...
function userHas(user,required_permission)
{
  if( global.config.deployment=='devsite'
   || global.config.deployment=='laptop') 
    required_permission = "dev:"+required_permission;
...
function checkPermissionJson(required_permission) {
  return function(req,res,next)
  {
    ...
    console.log(required_permission,req.user.permissions)
    return res.status(400).json({error:"Insufficient privileges. Need "+required_permission+" have "+req.user.permissions.join(',')})
  }
}
